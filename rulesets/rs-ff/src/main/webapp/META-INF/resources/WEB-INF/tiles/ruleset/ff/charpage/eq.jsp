...
<div class="ffEquipmentFullWidth" data-items="true">
    ...
    <c:forEach var="item" items="${data.items}"><c:if test="${!isFirst}">,</c:if>
    ...
    <c:if test="${not empty data.codewords}">
        <span class="ffMainAttribute ffCodewords"><spring:message code="page.ff.codewords" /></span>
        <c:forEach items="${data.codewords}" var="codeword" varStatus="stat">${codeword}<c:if test="${!stat.last}">,</c:if></c:forEach>
    </c:if>
