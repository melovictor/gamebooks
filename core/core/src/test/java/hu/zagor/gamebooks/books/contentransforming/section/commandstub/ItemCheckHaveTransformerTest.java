...
import hu.zagor.gamebooks.books.contentransforming.section.stub.itemcheck.ItemCheckHaveTransformer;
...
/**
 * Unit test for class {@link ItemCheckHaveTransformer}.
 * @author Tamas_Szekeres
 */
@Test
public class ItemCheckHaveTransformerTest {

    private ItemCheckHaveTransformer underTest;
    ...
    public void setUpClass() {
        ...
        underTest = new ItemCheckHaveTransformer();
    }
    ...
    public void testTransformWhenHaveEquippedIsNullShouldSetItAsWell() {
        // GIVEN
        expect(parent.parseParagraphData(positionCounter, node)).andReturn(paragraphData);
        command.setHave(paragraphData);
        expect(command.getHaveEquipped()).andReturn(null);
    ...
    public void testTransformWhenHaveEquippedIsNotNullShouldOnlySetHave() {
        ...
        expect(parent.parseParagraphData(positionCounter, node)).andReturn(paragraphData);
        command.setHave(paragraphData);
        expect(command.getHaveEquipped()).andReturn(paragraphData);
        mockControl.replay();
        // WHEN
        underTest.transform(parent, node, command, positionCounter);
        // THEN
