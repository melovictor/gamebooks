 ...
 */
public class ItemCheckHaveTransformer extends AbstractCommandSubTransformer<ItemCheckCommand> {
    ...
    protected void doTransform(final BookParagraphDataTransformer parent, final Node node, final ItemCheckCommand command,
        ...
        final ParagraphData haveData = parent.parseParagraphData(positionCounter, node);
        command.setHave(haveData);
        if (command.getHaveEquipped() == null) {
            command.setHaveEquipped(haveData);
        }
    }
